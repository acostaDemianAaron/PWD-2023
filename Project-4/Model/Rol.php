...
class Rol
{
    ...
    //Methods
    public function setValues($idRol, $rolDescripcion)
    {
        $this->setIdRol($idRol);
    ...
    public function Load()
    {
        ...
        if ($database->Start()) {
            ...
            if ($status > 0) {
                ...
                $this->setValues($row['idrol'], $row['roldescripcion']);
                $res = true;
            }
        } else {
    ...
    public function Insert()
    {
        $res = false;
        $database = new Database();
        $query = "INSERT INTO rol(roldescripcion) VALUES('" . $this->getRolDescripcion() . "')";
        if ($database->Start()) {
    ...
    public function Modify()
    {
        $res = false;
        $database = new Database();
        $query = "UPDATE rol SET roldescripcion = '{$this->getRolDescripcion()}' WHERE idrol = '" . $this->getIdRol() . "'";
        if ($database->Start()) {
